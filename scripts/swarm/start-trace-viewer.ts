...
export function startTraceViewer() {
  ...
  try {
    const running = execSync("docker ps -q -f name=hlbw-jaeger")
      .toString()
      .trim();
    if (running) {
    ...
    const stopped = execSync("docker ps -a -q -f name=hlbw-jaeger")
      .toString()
      .trim();
    if (stopped) {
      ...
      execSync(
        "docker run -d --name hlbw-jaeger -p 16686:16686 -p 4318:4318 jaegertracing/all-in-one:latest",
      );
    }
    console.log(
      "Jaeger trace viewer is now available at http://localhost:16686",
    );
  } catch (err: any) {
