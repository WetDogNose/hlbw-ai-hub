 ...
 * Jaeger:  http://localhost:16686  (service: wot-box-swarm)
 */
...
const DEMO_TASKS = [
  {
    ...
    description: "Add a UserPreferences model with theme, language, and timezone fields.",
    priority: 1,
    metadata: {},
    dependencies: [],
  },
  {
    ...
    description: "Implement GET/PUT routes for user preferences with auth guard.",
    priority: 2,
    metadata: {},
    dependencies: [],
  },
  {
    ...
    description: "React component with form inputs for theme, language, timezone. Uses SWR.",
    priority: 2,
    metadata: {},
    dependencies: [],
  },
  {
    ...
    description: "Jest tests covering auth, validation, and CRUD for the preferences API.",
    priority: 3,
    metadata: {},
    dependencies: [],
  },
...
async function simulateWorkerLifecycle(taskId: string, taskTitle: string, branchName: string) {
  ...
  return tracer.startActiveSpan(`Worker:lifecycle`, async (rootSpan) => {
    ...
    // Phase 1 — Spawn
    const worker = await tracer.startActiveSpan("Worker:spawn", async (span) => {
    ...
    await tracer.startActiveSpan("Worker:execute", async (span) => {
      ...
      await updateWorkerStatus(worker.id, WorkerStatus.Running, { runtimeId: `demo-container-${worker.id}` });
    ...
    await tracer.startActiveSpan("Worker:complete", async (span) => {
      ...
      await updateTaskStatus(taskId, TaskStatus.Completed, `worker-${worker.id}`);
      await appendAudit({
        actor: `demo-worker`,
...
async function main() {
  ...
  console.log('Service name: "wot-box-swarm"\n');

  // ---- Master trace: entire orchestration ----
  await tracer.startActiveSpan("MasterAgent:orchestrate", async (masterSpan) => {
    ...
    for (const t of DEMO_TASKS) {
      const created = await tracer.startActiveSpan("MasterAgent:registerTask", async (span) => {
    ...
    if (secondTask) {
      ...
      const { saveState } = await import("./state-manager");
    ...
    if (firstTask) {
      console.log(`   ➡️  Arbiter selected: ${firstTask.id} (${firstTask.title})`);
    }
    ...
    await tracer.startActiveSpan("Watchdog:healthCheck", async (span) => {
      ...
      const completed = postState.workers.filter((w) => w.status === WorkerStatus.Completed).length;
      ...
      console.log(`   Workers: ${active} active, ${completed} completed, ${postState.workers.length} total`);
      span.end();
    });

    // Step 5: Retention cleanup
    console.log("\n🧹 Step 5: Running retention cleanup...");
    await tracer.startActiveSpan("Watchdog:retentionCleanup", async (span) => {
      ...
      console.log(`   Cleaned up ${result.removedTasks} tasks, ${result.removedWorkers} workers.`);
      span.end();
    });
  ...
  console.log('   📊 http://localhost:16686  →  Service: "wot-box-swarm"\n');
  ...
  console.log("   • Worker:lifecycle → Worker:spawn → Worker:execute → Worker:complete");
  console.log("   • Worker:readFiles / Worker:applyEdits / Worker:runTests  (nested work)");
