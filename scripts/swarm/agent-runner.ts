        ...
        tools: [
            {
                functionDeclarations: [
                    ...
                    },
                    {
                        ...
                        parameters: {
                            type: SchemaType.OBJECT,
                            properties: { filePath: { type: SchemaType.STRING }, content: { type: SchemaType.STRING } },
    ...
    for (let i = 0; i < 30; i++) {
        try {
            ...
            if (!functionCalls || functionCalls.length === 0) {
                nextPrompt = "Please continue working or output exactly 'DONE' if you are completely finished.";
                continue;
            }

            let toolResponses: any[] = [];
            for (const call of functionCalls) {
                await tracer.startActiveSpan(`Tool:${call.name}`, async (toolSpan) => {
                    const args = call.args as Record<string, any>;
                    console.log(`> CALLING TOOL: ${call.name} with ${JSON.stringify(args).slice(0, 100)}...`);
                    try {
                        toolSpan.setAttribute("tool.args", JSON.stringify(args));
                        if (call.name === "read_file") {
                            const content = fs.readFileSync(args.filePath, "utf8");
                            console.log(`> READ ${args.filePath} (${content.length} bytes)`);
                            toolSpan.setAttribute("tool.content_length", content.length);
                            toolResponses.push({ functionResponse: { name: call.name, response: { content: content.slice(0, 20000) } } });
                        } else if (call.name === "write_file") {
                            fs.mkdirSync(path.dirname(args.filePath), { recursive: true });
                            fs.writeFileSync(args.filePath, args.content, "utf8");
                            console.log(`> WROTE ${args.filePath}`);
                            toolSpan.setAttribute("tool.content_length", String(args.content).length);
                            toolResponses.push({ functionResponse: { name: call.name, response: { success: true } } });
                        } else if (call.name === "exec_command") {
                            try {
                                const output = execSync(args.command, { encoding: "utf8", stdio: ['pipe', 'pipe', 'pipe'] });
                                console.log(`> EXEC ${args.command} STDOUT:\n${output}`);
                                toolSpan.setAttribute("tool.output", output.slice(0, 500));
                                toolResponses.push({ functionResponse: { name: call.name, response: { output: output.slice(0, 20000) } } });
                            } catch (execErr: any) {
                                const errorOutput = execErr.stderr?.toString() || execErr.stdout?.toString() || execErr.message;
                                console.log(`> EXEC ${args.command} FAILED:\n${errorOutput}`);
                                toolSpan.recordException(execErr);
                                toolResponses.push({ functionResponse: { name: call.name, response: { error: errorOutput } } });
                            }
                        }
