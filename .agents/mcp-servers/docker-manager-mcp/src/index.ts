...
const BuildImageArgsSchema = z.object({
  contextPath: z
    .string()
    .describe("Absolute path to the directory containing the Dockerfile"),
  imageName: z
    .string()
    .describe("Name/tag for the built image (e.g. hlbw-swarm-worker:latest)"),
});
...
const RunContainerArgsSchema = z.object({
  ...
  envKeys: z
    .record(z.string())
    .describe("Environment variables, object form { 'GEMINI_API_KEY': 'val' }"),
  command: z
    .array(z.string())
    .describe("The command array to run (e.g. ['bash', '-c', 'script'])"),
  extraBinds: z
    .array(z.string())
    .optional()
    .describe("Extra bind mounts in 'host:container' format"),
});
    ...
    tools: [
      {
        name: "build_image",
        description:
          "Builds a Docker image from a local context directory that contains a Dockerfile.",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        name: "run_container",
        description:
          "Runs an isolated Docker container in the background with a volume mount.",
        inputSchema: {
          type: "object",
          properties: {
            imageName: { type: "string" },
      ...
      },
      {
        ...
        description:
          "Retrieves the output payload of the containerized sub-agent.",
        inputSchema: {
          type: "object",
          properties: { containerId: { type: "string" } },
          required: ["containerId"],
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    switch (request.params.name) {
      case "build_image": {
        const { contextPath, imageName } = BuildImageArgsSchema.parse(
        ...
        return {
          content: [
            {
              type: "text",
              text: `Image ${imageName} built successfully from ${contextPath}`,
            },
          ],
        };
      }
      ...
      case "get_container_status": {
        const { containerId } = ContainerIdSchema.parse(
          request.params.arguments,
        );
      ...
      case "stop_container": {
        ...
        return {
          content: [
            {
              type: "text",
              text: `Container ${containerId} completely removed.`,
            },
          ],
        };
      }
