...
1. Find any unstructured, ambiguous, or legacy agent instructions (like 'Hey agent', 'Agent: remember to') and rewrite them into the precise XML structures defined below.
2. Evaluate ANY existing <agent_directive>, <agent_instruction>, or <agent_hint> tags.
...
3. Only modify the agent hints, instructions, and directives.
        ...
        for filepath in glob.glob(os.path.join(workspace_root, pattern), recursive=True):
            ...
            try:
                ...
                for i, line in enumerate(lines):
                    ...
                    if "hey agent" in lower_line or "agent: remember" in lower_line or "@agent" in lower_line:
                        if "<agent_" not in line:
                            legacy_found = True
                            results["legacy_warnings"].append(
                                f"File: {filepath}:{i+1} - Found unstructured agent guidance."
                            )
                
                # If there are existing tags, they might need conflict/loop resolution
                has_tags = "<agent_" in original_content
                ...
                if legacy_found or has_tags:
                    if mode == "fix":
                        ...
                        # Extract the tags so we can build a graph
                        directives = re.findall(r'<agent_directive.*?</agent_directive>', original_content, re.DOTALL)
                        instructions = re.findall(r'<agent_instruction.*?</agent_instruction>', original_content, re.DOTALL)
                        hints = re.findall(r'<agent_hint.*?</agent_hint>', original_content, re.DOTALL)
                        ...
                        if directives or instructions or hints:
                            ...
                            file_blocks.extend(directives)
                            file_blocks.extend(instructions)
                            file_blocks.extend(hints)
                            results["extracted_tags"].append("\n".join(file_blocks))
                
                # Tally correct syntaxes
                results["valid_directives"] += len(re.findall(r'<agent_directive', original_content))
                results["valid_instructions"] += len(re.findall(r'<agent_instruction', original_content))
                results["valid_hints"] += len(re.findall(r'<agent_hint', original_content))
