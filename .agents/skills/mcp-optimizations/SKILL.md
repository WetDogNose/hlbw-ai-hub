...
## 4. Production Database Interaction (`postgres-prod`)
When instructed to query production data, check user states, or troubleshoot data issues in production.
* **Instead:** Call `postgres-prod_query` from the `postgres-prod` server.
* **Why:** This directly and safely connects to the `wot-box` database without needing to manually run container commands or proxy connections.

## 5. Google Cloud Logs and Traces (`gcp-logging`, `gcp-trace`)
...
* **Instead:** Call tools like `read_gcp_logs` from the `gcp-logging-mcp` server, or `get_trace_details` / `list_recent_traces` from the `gcp-trace-mcp` server.
* **Why:** The native `gcloud` CLI commands are slow, error-prone, and return massive untruncated payloads. These MCPs are heavily optimized for AI extraction speed and data structure.
