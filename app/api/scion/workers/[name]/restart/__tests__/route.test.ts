// Pass 22 — POST /api/scion/workers/[name]/restart unit tests.
...
function req(name: string): Request {
  return new Request(
    `http://localhost/api/scion/workers/${encodeURIComponent(name)}/restart`,
    { method: "POST" },
  );
}
...
describe("POST /api/scion/workers/[name]/restart", () => {
  ...
  it("200 + audit on success", async () => {
    getIapUserMock.mockResolvedValue(admin);
    const res = await POST(req("hlbw-hub-local"), {
      params: { name: "hlbw-hub-local" },
    });
